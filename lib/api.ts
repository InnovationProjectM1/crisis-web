...
const mapClassificationToCategory = (classifier?: ApiClassifier): "need" | "resource" | "alert" => {
  ...
  if (group.includes('need') || subGroup.includes('need') || 
      group.includes('require') || subGroup.includes('require') ||
      group.includes('help') || subGroup.includes('help')) {
    return "need";
  }
  
  if (group.includes('resource') || subGroup.includes('resource') || 
      group.includes('offer') || subGroup.includes('offer') ||
      group.includes('available') || subGroup.includes('available')) {
    return "resource";
  }
...
const mapDifficultyToUrgency = (difficulty?: string): "low" | "medium" | "high" => {
  if (!difficulty) return "medium";
  
  const diff = difficulty.toLowerCase();
  if (diff.includes('high') || diff.includes('urgent') || diff.includes('critical')) {
    return "high";
  }
...
class ApiService {
  ...
  async getTweet(id: number): Promise<ApiTweet> {
    const response = await fetch(buildApiUrl(`${API_CONFIG.ENDPOINTS.TWEETS}/${id}`));
    if (!response.ok) {
  ...
  async createTweet(tweetId: number, tweetText: string, timestamp?: Date): Promise<ApiTweet> {
  ...
  async getTweetStatistics(): Promise<TweetStatistics> {
    try {
      const response = await fetch(buildApiUrl(API_CONFIG.ENDPOINTS.TWEET_STATISTICS));
      if (!response.ok) {
  ...
  async getClassifier(tweetId: number): Promise<ApiClassifier> {
    const response = await fetch(buildApiUrl(`${API_CONFIG.ENDPOINTS.CLASSIFIERS}/${tweetId}`));
    if (!response.ok) {
  ...
  async createClassifier(classifier: Omit<ApiClassifier, 'tweet'>): Promise<ApiClassifier> {
    const response = await fetch(buildApiUrl(API_CONFIG.ENDPOINTS.CLASSIFIERS), {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  async updateClassifier(tweetId: number, classifier: Partial<Omit<ApiClassifier, 'tweet_id' | 'tweet'>>): Promise<ApiClassifier> {
    const response = await fetch(buildApiUrl(`${API_CONFIG.ENDPOINTS.CLASSIFIERS}/${tweetId}`), {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
  ...
  async getGroupStatistics(): Promise<GroupStatistics[]> {
    try {
      const response = await fetch(buildApiUrl(API_CONFIG.ENDPOINTS.GROUP_STATISTICS));
      if (!response.ok) {
  ...
  async getDifficultyStatistics(): Promise<DifficultyStatistics[]> {
    try {
      const response = await fetch(buildApiUrl(API_CONFIG.ENDPOINTS.DIFFICULTY_STATISTICS));
      if (!response.ok) {
  ...
  async getTrendData(timeRange: '24h' | '7d' | '30d' = '24h'): Promise<Array<{
    label: string;
    timestamp: Date;
    Needs: number;
    Resources: number;
    ...
    try {
      ...
      for (let i = 0; i < dataPoints; i++) {
        ...
        const resources = periodTweets.filter(t => t.category === 'resource').length;
        const volunteers = periodTweets.filter(t => t.urgency === 'high').length;
        ...
        data.push({
          ...
          change: i > 0 ? Math.round(((needs - data[i-1]?.Needs || 0) / Math.max(data[i-1]?.Needs || 1, 1)) * 100) : 0
        });
      }
  ...
  async getRegionData(): Promise<Array<{
    id: string;
    name: string;
    ...
    try {
      ...
      const regions = Array.from(locationMap.entries()).map(([location, locationTweets], index) => {
        // Utiliser les coordonnées de configuration
        const lat = API_CONFIG.DEFAULT_COORDINATES.LAT_BASE[index % API_CONFIG.DEFAULT_COORDINATES.LAT_BASE.length] + (Math.random() - 0.5) * API_CONFIG.DEFAULT_COORDINATES.RANDOM_OFFSET;
        const lng = API_CONFIG.DEFAULT_COORDINATES.LNG_BASE[index % API_CONFIG.DEFAULT_COORDINATES.LNG_BASE.length] + (Math.random() - 0.5) * API_CONFIG.DEFAULT_COORDINATES.RANDOM_OFFSET;

        // Analyser les tweets par catégorie et urgence
        const needTweets = locationTweets.filter(t => t.category === 'need');
        const resourceTweets = locationTweets.filter(t => t.category === 'resource');

        const needs = [
          { type: "medical", count: needTweets.filter(t => t.text.toLowerCase().includes('medical') || t.text.toLowerCase().includes('doctor')).length, urgency: "high" },
          { type: "food", count: needTweets.filter(t => t.text.toLowerCase().includes('food') || t.text.toLowerCase().includes('water')).length, urgency: "medium" },
          { type: "shelter", count: needTweets.filter(t => t.text.toLowerCase().includes('shelter') || t.text.toLowerCase().includes('housing')).length, urgency: "medium" },
        ].filter(resource => resource.count > 0);

        const resources = [
          { type: "volunteers", count: resourceTweets.filter(t => t.text.toLowerCase().includes('volunteer') || t.text.toLowerCase().includes('help')).length, urgency: "low" },
          { type: "supplies", count: resourceTweets.filter(t => t.text.toLowerCase().includes('supply') || t.text.toLowerCase().includes('available')).length, urgency: "medium" },
        ].filter(resource => resource.count > 0);
      ...
      }).filter(region => region.needs.length > 0 || region.resources.length > 0);
  ...
  async getHeatmapData(): Promise<{
    ...
    try {
      ...
      const regions = Array.from(locationMap.entries()).map(([location, locationTweets], index) => {
        ...
        const needTweets = locationTweets.filter(t => t.category === 'need').length;
        const resourceTweets = locationTweets.filter(t => t.category === 'resource').length;
        const urgentTweets = locationTweets.filter(t => t.urgency === 'high').length;

        // Normaliser les intensités (0-1)
        const maxTweetsPerLocation = Math.max(...Array.from(locationMap.values()).map(tweets => tweets.length));
        const needsIntensity = totalTweets > 0 ? Math.min(1, (needTweets + urgentTweets * 0.5) / Math.max(maxTweetsPerLocation, 1)) : 0;
        const resourcesIntensity = totalTweets > 0 ? Math.min(1, resourceTweets / Math.max(maxTweetsPerLocation, 1)) : 0;
